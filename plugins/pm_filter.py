...
async def advantage_spell_chok(client, msg):
    ...
    # Ensure g_s and gs_parsed are handled safely
    g_s = await search_gagala(query)
    if not g_s:
        g_s = await search_gagala(msg.text)
    
    if not g_s:
        reqst_gle = quote_plus(query)
    ...
    movielist = []
    if gs_parsed:
        tasks = [get_poster(mov.strip(), bulk=True) for mov in gs_parsed]
        imdb_s_results = await asyncio.gather(*tasks)
        for imdb_s in imdb_s_results:
            if imdb_s:
                movielist += [movie.get('title') for movie in imdb_s]
    
    movielist = list(set(movielist))  # removing duplicates with set
    if not movielist:
        reqst_gle = quote_plus(query)
        button = [[InlineKeyboardButton("Gᴏᴏɢʟᴇ", url=f"https://www.google.com/search?q={reqst_gle}")]]
        if NO_RESULTS_MSG:
    ...
    btn = [[InlineKeyboardButton(text=movie.strip(), callback_data=f"spolling#{reqstr1}#{k}") for k, movie in enumerate(movielist)]]
